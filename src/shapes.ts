 ...
 * of days, the hour is a clock sweep, and the card about how long they went
 * quiet gets a nearly empty frame with one mark in it.
 */
...
function part(tag: string, attrs: Attrs, tone: 'a' | 'b' | 'line', index: number): SVGElement {
...
function ring(cx: number, cy: number, r: number, i: number): SVGElement {
  return part('circle', { cx, cy, r }, 'line', i);
}

/**
 * A rotated shape goes inside a group that carries the rotation, because the
 * entry animation sets `transform` in CSS and a CSS transform overrides an
 * element's own transform attribute outright.
 */
function turned(el: SVGElement, degrees: number): SVGElement {
  const group = node('g', { transform: `rotate(${degrees} 50 50)` });
  group.append(el);
  return group;
}
...
function build(kind: ShapeKind, uid: string): SVGElement[] {
  switch (kind) {
    // The curtain going up: rings opening out of the centre.
    case 'rings':
      return [16, 28, 42, 58, 76].map((r, i) => ring(50, 38, r, i));

    // One enormous count, and the small thing orbiting it.
    case 'orbit':
      return [
        part('circle', { cx: 12, cy: 74, r: 52 }, 'a', 0),
        part('circle', { cx: 80, cy: 20, r: 11 }, 'b', 1),
        ring(80, 20, 20, 2),
      ];

    // Words stacking up. Hung from the top edge, because this card sets its
    // figure at the bottom of the frame.
    case 'bars':
      return [26, 44, 32, 54, 36, 48, 24].map((h, i) =>
        part('rect', { x: 4 + i * 14, y: 0, width: 9, height: h, rx: 4.5 }, i % 2 ? 'b' : 'a', i)
      );

    // Two people, and the overlap.
    case 'twin':
      return [
        part('circle', { cx: 30, cy: 50, r: 33 }, 'a', 0),
        part('circle', { cx: 70, cy: 50, r: 33 }, 'b', 1),
      ];

    // A clock face, pushed off the centre of the frame so the hand never runs
    // through the hour it is pointing at.
    case 'arc':
      return [
        part('circle', { cx: 24, cy: 22, r: 26 }, 'line', 0),
        part('path', { d: 'M 24 22 L 24 2', fill: 'none' }, 'line', 1),
        part('path', { d: 'M 24 22 L 40 30', fill: 'none' }, 'line', 2),
        part('circle', { cx: 24, cy: 22, r: 2.6 }, 'a', 3),
        part('circle', { cx: 78, cy: 82, r: 20 }, 'a', 4),
      ];

    // One mark per day, unbroken.
    case 'grid': {
      const dots: SVGElement[] = [];
      for (let row = 0; row < 5; row += 1) {
        for (let col = 0; col < 9; col += 1) {
          dots.push(
            part('circle', { cx: 8 + col * 10.5, cy: 52 + row * 12, r: 2.4 }, 'a', row * 9 + col)
          );
        }
      }
      return dots;
    ...
    // Hers: the one soft, unruled shape in the set.
    case 'blob':
      return [
        part(
          'path',
    ...
    // Laughing: everything going outward at once.
    case 'burst': {
    ...
    // A single message, held.
    case 'halo':
      return [part('circle', { cx: 50, cy: 46, r: 40 }, 'a', 0), ring(50, 46, 47, 1)];
    ...
    case 'moon': {
      ...
      defs.append(mask);
      const disc = part('circle', { cx: 58, cy: 36, r: 32, mask: `url(#moon-${uid})` }, 'a', 0);
      return [defs, disc, part('circle', { cx: 22, cy: 74, r: 1.8 }, 'b', 1), part('circle', { cx: 32, cy: 86, r: 1.2 }, 'b', 2)];
    }

    // A conversation that kept turning back on itself.
    case 'spiral':
    ...
    // The quiet one. Almost nothing in the frame, on purpose.
    case 'sparse':
      return [
        part('path', { d: 'M 6 62 L 58 62', fill: 'none' }, 'line', 0),
        part('circle', { cx: 84, cy: 32, r: 3 }, 'a', 1),
      ];

    // Her rose, opening.
    case 'bloom':
      return [0, 1, 2, 3, 4].map((i) =>
        turned(part('ellipse', { cx: 50, cy: 50, rx: 40, ry: 15 }, i % 2 ? 'b' : 'a', i), i * 36)
      );
  }
}
